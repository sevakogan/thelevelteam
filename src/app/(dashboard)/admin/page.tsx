    ...
    return (
      <div className="min-h-screen bg-brand-dark flex items-center justify-center p-6">
        <form
          ...
          className="w-full max-w-sm p-8 rounded-ios-lg border border-separator bg-brand-card/50 backdrop-blur-sm"
        >
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-lg bg-brand-darker border border-separator text-foreground placeholder-brand-muted focus:outline-none focus:border-accent-blue mb-4"
          />
          <button
            type="submit"
            className="w-full py-3 rounded-lg bg-gradient-to-r from-accent-blue to-accent-purple text-foreground font-medium hover:shadow-glow transition-shadow"
          >
            Login
          </button>
        </form>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <a
              href="/"
              className="px-4 py-2 rounded-lg border border-separator text-brand-muted hover:text-foreground transition-colors text-sm"
            >
              View Site
            </a>
            <button
        ...
        <AnimatePresence>
          {(editing !== null || !editing) && editing !== undefined && form.name !== undefined && (
            <motion.form
              ...
              onSubmit={handleSave}
              className="mb-8 p-6 rounded-ios-lg border border-separator bg-brand-card/30"
            >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <input
                  ...
                  className="px-4 py-3 rounded-lg bg-brand-darker border border-separator text-foreground placeholder-brand-muted focus:outline-none focus:border-accent-blue"
                />
                <input
                  required
                  value={form.slug}
                ...
                <input
                  ...
                  placeholder="Tagline"
                  className="px-4 py-3 rounded-lg bg-brand-darker border border-separator text-foreground placeholder-brand-muted focus:outline-none focus:border-accent-blue md:col-span-2"
                />
                <input
                  value={form.live_url}
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <input
                    ...
                    className="w-20 px-3 py-2 rounded-lg bg-brand-darker border border-separator text-foreground focus:outline-none focus:border-accent-blue"
                  />
              ...
              <div className="flex gap-3 mt-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2.5 rounded-lg bg-gradient-to-r from-accent-blue to-accent-purple text-foreground text-sm font-medium hover:shadow-glow transition-shadow disabled:opacity-50"
                >
                ...
                {editing && (
                  <button
                    type="button"
                    onClick={cancelEdit}
                    className="px-6 py-2.5 rounded-lg border border-separator text-brand-muted hover:text-foreground text-sm transition-colors"
                  >
                    Cancel
                  </button>
                )}
        ...
        <div className="space-y-3">
          {companies.map((company) => (
            <motion.div
              key={company.id}
              layout
              className="flex items-center justify-between p-4 rounded-xl border border-separator bg-brand-card/30 hover:bg-brand-card/50 transition-colors"
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border border-separator text-brand-muted hover:text-foreground text-xs transition-colors"
                >
                  Edit
                </button>
                <button
